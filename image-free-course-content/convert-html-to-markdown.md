...
"A diagram illustrating the conversion of Word documents to Markdown.  The top displays the text 'Word to Markdown' in a light blue banner.  Below, a stylized blue square representing the Microsoft Word app icon with a large 'W' is shown.  An arrow points from this to a small grey icon labeled 'Pandoc' (partially visible), suggesting it as the conversion tool. Another arrow points from Pandoc to a black square with the letter 'M' and a downward-pointing arrow inside, symbolizing the Markdown output."]](https://youtu.be/HPSK7q13-40)
