...
A light-skinned man with short brown hair is shown from the chest up, gesturing with his right hand towards a computer screen displaying Python code. The man has a slightly quizzical expression.  The screen shows a code snippet defining a function called "get_flight_info," which takes parameters for origin and destination airports. A red arrow points to this part of the code.  The title at the top of the image reads, in large, bold white sans-serif letters, "FUNCTIONS EXPLAINED." The background is dark bluish-gray. The man is wearing a dark gray zip-up jacket and a black smartwatch. The code editor uses a dark theme, with the code in light colors on a dark background.]](https://youtu.be/aqdWSYWC_LI)
